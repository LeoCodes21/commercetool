...
func (g *GenerateGlobalScriptCommand) Run(_ *Context) error {
	...
	listFile, err := os.Open(filepath.Join(exPath, "global_visualpart_list.txt"))
	if err != nil {
		return err
	}
	...
	scriptLines := []string{
		"# Generated by CommerceTool by heyitsleo",
		fmt.Sprintf("# Time: %s", time.Now().Format(time.RFC1123Z)),
		fmt.Sprintf("# Car: %s", g.CarName),
	}
